#ifndef LIMITED_PLANES_DATA_H
#define LIMITED_PLANES_DATA_H
...
namespace dte3607::coldet::mysolvers::limited_planes_data {
  ...
  void handleCollision(CollisionsOsFp& C_OsFp, [[maybe_unused]] CollisionsOss& C_Oss,
                       ...
                       [[maybe_unused]] Spheres& O_S, [[maybe_unused]] FixedSpheres const& F_S,
                       [[maybe_unused]] FixedPlanes const&        F_P,
                       [[maybe_unused]] FixedLimitedPlanes const& F_LP, Distribution& d_x,
                       Distribution& d_z) {
    ...
    std::printf("%f|%f\n", c.s->point()[0], c.s->point()[2]);
    int const x_index = (int)blaze::floor(c.s->point()[0]) + 25;
    int const z_index = (int)blaze::floor(c.s->point()[2]) + 25;
    if (x_index >= 0 && x_index < 50 && z_index >= 0 && z_index < 50) {
      d_x[x_index]++;
      d_z[z_index]++;
    }
  ...
  // requires concepts::scenario_fixtures::SolverFixtureGaltonLimitedPlane<SolverDevFixture_T> bool
  bool solve(fixture& scenario, NS timestep) {
    ...
    Distribution&             d_x  = scenario->xdistribution();
    Distribution&             d_z  = scenario->zdistribution();
