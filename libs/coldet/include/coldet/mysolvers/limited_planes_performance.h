#ifndef LIMITED_PLANES_PERFORMANCE_H
#define LIMITED_PLANES_PERFORMANCE_H
...
namespace dte3607::coldet::mysolvers::limited_planes_performance {
  ...
  void detectInitialCollisions(Spheres const& O_S, FixedSpheres const& F_S, FixedPlanes const& F_P,
                               ...
                               VT const& pyr_top) {
    InstrumentationTimer timer("Detect init col");
    ...
    for (auto& s : O_S) {
      if (s->state() != States::Resting) {
        for (auto& s2 : O_S) {
          if (s2->state() != States::Resting) {
            ...
            if (collision) {
              ...
              CollisionOss c_Oss{c_tp, s.get(), s2.get(), false};
        ...
        for (auto& fs : F_S) {
          if (s->point()[1] < pyr_top + s->radius()) {
            ...
            if (collision) {
              auto const    c_dt = std::chrono::duration_cast<NS>(collision.value() * timestep);
              auto const    c_tp = t_0 + c_dt;
              CollisionOsFs c_OsFs{c_tp, s.get(), fs.get(), false};
        ...
        for (auto& p : F_P) {
          auto const collision = mechanics::detectCollisionSphereFixedPlane(s.get(), p.get(), trajectories);
          if (collision) {
            auto const    c_dt = std::chrono::duration_cast<NS>(collision.value() * timestep);
            auto const    c_tp = t_0 + c_dt;
            CollisionOsFp c_OsFp{c_tp, s.get(), p.get(), false};
        ...
        for (auto& lp : F_LP) {
          auto const collision
            = mechanics::detectCollisionSphereFixedLimitedPlane(s.get(), lp.get(), trajectories);
          if (collision) {
            auto [x, fromSide]  = collision.value();
            auto const     c_dt = std::chrono::duration_cast<NS>(x * timestep);
            auto const     c_tp = t_0 + c_dt;
            CollisionOsFlp c_OsFlp{c_tp, s.get(), lp.get(), false};
  ...
  void detectNewCollisions(Sphere* const s, FixedLimitedPlane* const lp, Trajectories const& trajectories,
                           CollisionsOsFlp& C_OsFlp, TP const& t_0, NS const& timestep) {
    InstrumentationTimer timer("Detect");
  ...
  void simulate(Sphere* s, OsFpAttachments const& attachmentsOsFp, Trajectories const& trajectories,
    ...
    InstrumentationTimer timer("Simulate");
  ...
  void response(CollisionOss& c, OsFpAttachments& attachmentsOsFp, Trajectories& trajectories, V3 const force,
    ...
    InstrumentationTimer timer("Response");
  ...
  void searchForNewCollisions(CollisionOss& c, Spheres const& O_S, FixedSpheres const& F_S,
                              ...
                              NS const& timestep) {
    InstrumentationTimer timer("Search for new collisions");

    for (auto& s_other : O_S) {
      if (c.s1 != s_other.get()) detectNewCollisions(c.s1, s_other.get(), trajectories, C_Oss, t_0, timestep);
      if (c.s2 != s_other.get()) detectNewCollisions(c.s2, s_other.get(), trajectories, C_Oss, t_0, timestep);
    }
    for (auto& p : F_P) {
  ...
  void searchForNewCollisions(CollisionOsFs& c, Spheres const& O_S, FixedSpheres const& F_S,
    ...
    for (auto& s_other : O_S)
      if (c.os != s_other.get()) detectNewCollisions(c.os, s_other.get(), trajectories, C_Oss, t_0, timestep);
    for (auto& p : F_P)
  ...
  void searchForNewCollisions(CollisionOsFp& c, Spheres const& O_S, FixedSpheres const& F_S,
    ...
    for (auto& s_other : O_S)
      if (c.s != s_other.get()) detectNewCollisions(c.s, s_other.get(), trajectories, C_Oss, t_0, timestep);
    for (auto& p : F_P)
  ...
  void handleCollision(CollisionsOss& C_Oss, CollisionsOsFp& C_OsFp, CollisionsOsFs& C_OsFs,
                       ...
                       V3 const force, NS const& timestep, TP const& t_0, Spheres& O_S,
                       FixedSpheres const& F_S, FixedPlanes const& F_P, FixedLimitedPlanes const& F_LP) {
    InstrumentationTimer timer("Handle collision");
  ...
  void handleCollision(CollisionsOsFp& C_OsFp, OsFpAttachments& attachmentsOsFp, Trajectories& trajectories,
                       NS const& timestep, TP const& t_0) {
    InstrumentationTimer timer("Handle collision");
  ...
  void sortAndMakeUnique(CollisionsOsFp& C_OsFp, CollisionsOss& C_Oss, CollisionsOsFs& C_OsFs,
    ...
    InstrumentationTimer timer("Sort and make unique");
  ...
  bool solve(fixture& scenario, NS timestep) {
    ...
    while (not C_OsFp.empty() || not C_Oss.empty() || not C_OsFs.empty() || not C_OsFlp.empty()) {
      ...
      switch (collisions.back().second) {
        ...
        case '2':
          ...
          break;
        case '3':
          handleCollision(C_OsFp, attachmentsOsFp, trajectories, timestep, t_0);
          scenario->counter()++;
          break;
        case '4':
    ...
    for (auto& s : O_S) {
      if (s->state() != States::Resting) {
        auto const dt = timestep - (s->timepoint() - t_0);
        simulate(s.get(), attachmentsOsFp, trajectories, dt, timestep, t_0);
        auto [ds_next_free, a_next_free] = mechanics::computeLinearTrajectory(s->velocity(), G, timestep);

        if (s->state() == States::Free)
          cache(s.get(), ds_next_free, a_next_free, trajectories);
        else if (s->state() == States::Sliding || s->state() == States::Rolling) {
          auto const plane = attachmentsOsFp.at(s.get());
          utils::state::detectStateChange(s.get(), plane, attachmentsOsFp, ds_next_free);

          if (s->state() == States::Sliding || s->state() == States::Rolling) {
            auto [ds_next_adj, a_next_adj]
              = mechanics::computeLinearTrajectory(s.get(), attachmentsOsFp, G, timestep);
            cache(s.get(), ds_next_adj, a_next_adj, trajectories);
          }
          else if (s->state() == States::Free)
            cache(s.get(), ds_next_free, a_next_free, trajectories);
        }
      }
    }

    return scenario->counter() >= O_S.size();
  }
}   // namespace dte3607::coldet::mysolvers::limited_planes_performance


#endif   // LIMITED_PLANES_PERFORMANCE_H
