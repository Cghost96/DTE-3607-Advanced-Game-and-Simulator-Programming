...
namespace app
{
  ...
  struct ScenarioWrapperBase {
    ...
    using SphereData = std::tuple<Frame3H, Vector3, ValueType>;

    // Frame - point - normal
    using PlaneData  = std::tuple<Frame3H, Point3, Vector3>;

    // Frame - point - u axis - v axis - thickness
    using LimitedPlaneData
      = std::tuple<Frame3H, Point3, Vector3, Vector3, ValueType>;
    ...
    {
      return numberOfSpheres() + numberOfPlanes() + numberOfLimitedPlanes()
             + numberOfBezierSurfaces();
    }
    size_t spheresOffset() const { return 0; }
    size_t planesOffset() const { return spheresOffset() + numberOfSpheres(); }
    size_t limitedPlanesOffset() const
    {
      return planesOffset() + numberOfPlanes();
    }
    size_t bezierSurfacesOffset() const
    {
      return limitedPlanesOffset() + numberOfLimitedPlanes();
    }
    ...
    virtual std::optional<LimitedPlaneData>
    limitedPlaneData(size_t obj_nr) const = 0;
    virtual std::optional<BezierSurfaceData>
    bezierSurfaceData(size_t obj_nr) const = 0;
  ...
  struct ScenarioWrapper : ScenarioWrapperBase {
    ...
    std::optional<BezierSurfaceData>
    bezierSurfaceData(size_t obj_nr) const final;
  };
  ...
  template <typename ConstructorLambda_T, typename SolverLambda_T,
            typename ScenarioPtr_T>
  size_t ScenarioWrapper<ConstructorLambda_T, SolverLambda_T,
                         ScenarioPtr_T>::numberOfLimitedPlanes() const
  {
    std::scoped_lock lock(m_mutex);
    if (not m_scenario_ptr) return {};
  ...
  template <typename ConstructorLambda_T, typename SolverLambda_T,
            typename ScenarioPtr_T>
  size_t ScenarioWrapper<ConstructorLambda_T, SolverLambda_T,
                         ScenarioPtr_T>::numberOfBezierSurfaces() const
  {
    std::scoped_lock lock(m_mutex);
    if (not m_scenario_ptr) return {};
                  ...
                  ScenarioPtr_T>::sphereData(size_t obj_nr) const
  {
    ...
    return detail::getSphereData<ScenarioWrapperBase::SphereData>(
      *m_scenario_ptr, obj_nr);
  }
  ...
  ScenarioWrapper<ConstructorLambda_T, SolverLambda_T,
                  ScenarioPtr_T>::planeData(size_t obj_nr) const
  {
    std::scoped_lock lock(m_mutex);
    if (not m_scenario_ptr) return {};

    return detail::getFixedPlaneData<ScenarioWrapperBase::PlaneData>(
      *m_scenario_ptr, obj_nr);
  }
  ...
  ScenarioWrapper<ConstructorLambda_T, SolverLambda_T,
                  ScenarioPtr_T>::limitedPlaneData(size_t obj_nr) const
  {
    std::scoped_lock lock(m_mutex);
    if (not m_scenario_ptr) return {};

    return detail::getFixedLimitedPlaneData<
        ScenarioWrapperBase::LimitedPlaneData>(*m_scenario_ptr, obj_nr);
  }
  ...
  ScenarioWrapper<ConstructorLambda_T, SolverLambda_T,
                  ScenarioPtr_T>::bezierSurfaceData(size_t obj_nr) const
  {
    std::scoped_lock lock(m_mutex);
    if (not m_scenario_ptr) return {};

    return detail::getFixedBezierSurfaceData<
      ScenarioWrapperBase::BezierSurfaceData>(*m_scenario_ptr, obj_nr);
  }
