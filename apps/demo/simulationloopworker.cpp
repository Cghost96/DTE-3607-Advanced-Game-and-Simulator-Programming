...
namespace app
{
  ...
  SimulationLoopWorker::SimulationLoopWorker(
    std::weak_ptr<ScenarioWrapperBase> scenario, NanoSeconds timestep)
  ...
  void SimulationLoopWorker::simulate()
  {
    ...
    auto const  benchmark_tp_start = std::chrono::system_clock::now();
    {
      ...
      if (std::cmp_equal(time_step.count(), 0)) {
        ...
        auto variable_time_step
          = std::chrono::duration_cast<NanoSeconds>(now - prev_timepoint);
    ...
    emit simulationFrameTimingsReady(
      int(std::chrono::duration_cast<MicroSeconds>(time_step).count()),
