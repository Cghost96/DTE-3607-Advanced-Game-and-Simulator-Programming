...
namespace app::detail
{
  ...
  template <typename Fixture_T>
  requires ns::HasSpheres<Fixture_T>
    size_t getNumberOfSpheres(Fixture_T const& s)
  {
  ...
  template <typename Fixture_T>
  requires ns::HasFixedPlanes<Fixture_T>
    size_t getNumberOfFixedPlanes(Fixture_T const& s)
  {
  ...
  template <typename Fixture_T>
  requires ns::HasFixedLimitedPlanes<Fixture_T>
    size_t getNumberOfFixedLimitedPlanes(Fixture_T const& s)
  {
  ...
  template <typename Fixture_T>
  requires ns::HasFixedBezierSurfaces<Fixture_T>
    size_t getNumberOfFixedBezierSurfaces(Fixture_T const& s)
  {
  ...
  std::optional<SphereData_T> getSphereData(Fixture_T const& /*s*/,
                                            size_t /*obj_nr*/)
  {
    return {};
  }
  ...
  template <typename SphereData_T, typename Fixture_T>
  requires ns::HasSpheres<Fixture_T> std::optional<SphereData_T>
  getSphereData(Fixture_T const& s, size_t obj_nr)
  {
    const auto& sphere = s.spheres()[obj_nr];

    return {{sphere->spaceObjectFrame().pSpaceFrameParent(),
             sphere->velocity(), sphere->radius()}};
  }

  template <typename FixedPlaneData_T, typename Fixture_T>
  std::optional<FixedPlaneData_T> getFixedPlaneData(Fixture_T const& /*s*/,
                                                    size_t /*obj_nr*/)
  {
    return {};
  }
  ...
  template <typename FixedPlaneData_T, typename Fixture_T>
  requires ns::HasFixedPlanes<Fixture_T> std::optional<FixedPlaneData_T>
  getFixedPlaneData(Fixture_T const& s, size_t obj_nr)
  {
    const auto& plane = s.fixedPlanes()[obj_nr];
    return {{plane->spaceObjectFrame().pSpaceFrameParent(), plane->point(),
             plane->normal()}};
  }

  template <typename FixedLimitedPlaneData_T, typename Fixture_T>
  std::optional<FixedLimitedPlaneData_T>
    ...
    getFixedLimitedPlaneData(Fixture_T const& s, size_t obj_nr)
  {
    ...
    return {{plane->spaceObjectFrame().pSpaceFrameParent(), plane->pointLocal(),
             plane->uAxisLocal(), plane->vAxisLocal(), plane->thickness()}};
  }

  template <typename FixedBezierSurfaceData_T, typename Fixture_T>
  std::optional<FixedBezierSurfaceData_T>
  ...
  getFixedBezierSurfaceData(Fixture_T const& s, size_t obj_nr)
  {
    ...
    return {
      {bez->spaceObjectFrame().pSpaceFrameParent(), bez->bezierSurface().m_C}};
  }
